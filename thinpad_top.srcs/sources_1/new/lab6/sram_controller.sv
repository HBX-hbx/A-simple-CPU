module sram_controller #(
    ...
    always_comb begin
        ...
        if (state == STATE_READ || state == STATE_READ_2) begin
            sram_data_t = 1'b1;
        end else if (state == STATE_WRITE || state == STATE_WRITE_2 || state == STATE_WRITE_3) begin
            sram_data_t = 1'b0;
    ...
    always_comb begin
        ...
        sram_oe_n = 1'b1;
        sram_we_n = 1'b1;
        if (state == STATE_WRITE || state == STATE_WRITE_3) begin
            sram_ce_n = 1'b0;
            sram_oe_n = 1'b1;
            sram_we_n = 1'b1;
        end
        ...
        else if (state == STATE_READ || state == STATE_READ_2) begin
            sram_ce_n = 1'b0;
            sram_oe_n = 1'b0;
            sram_we_n = 1'b1;
        end
            ...
            case (state)
                STATE_IDLE: begin
                    if (wb_stb_i && wb_cyc_i) begin
                        // wb_adr_i 非 4 的倍数，且要读多位时怎样处理？分两次读？
